...
import { getSecurityAlerts, subscribeToRealtimeStream, simulateSecurityAlert } from "../services/api";
...
const SOCDashboard = () => {
  ...
  const handleSimulateIncident = async () => {
    try {
      ...
      if (res?.success) {
        toast.success("Security Incident Simulation triggered!");
        await fetchAlerts();
      } else {
        toast.error(res?.message || "Failed to simulate incident");
      }
    } catch (err) {
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={simulating}
              className="px-4 py-2.5 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 hover:text-amber-200 border border-amber-500/40 transition flex items-center gap-2 text-xs font-mono font-bold cursor-pointer disabled:opacity-50 shadow-sm"
              title="Simulate Real-time SOC Incident"
            >
                  ...
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2 font-mono text-xs">
                      ...
                      {incident.isSimulated && (
                        <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-amber-500/20 text-amber-300 border border-amber-500/50 shadow-sm flex items-center gap-1 animate-pulse">
                          <span>⚡</span> [SIMULATED EVENT]
                        </span>
                      )}
                    </div>
            ...
            <div className="space-y-3 font-mono text-xs">
              {selectedIncident.isSimulated && (
                <div className="p-3 rounded-xl bg-amber-950/40 border border-amber-500/40 text-amber-300 text-xs flex items-center gap-2">
                  ...
                  <div>
                    <span className="font-bold block">[SIMULATED EVENT NOTICE]</span>
                    <span className="text-[11px] text-amber-300/80">This incident was generated by the SOC Incident Simulator for testing response playbooks. It does not represent live malicious blockchain activity.</span>
                  </div>
                </div>
              )}
